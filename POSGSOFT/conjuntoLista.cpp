...
void Universidad::diligenciarObservaciones(){ //Se encarga de guardar una observacion extra si se solicita
   ...
   cin >> codigoSelect;
   // Ciclo para recorrer las actas y ver si existe el acta con el numero dado
   for( i = 0; i < vectorActasTrabajos.size(); i++){
      if ( vectorActasTrabajos[i].getNumeroActa() == codigoSelect ){
   ...
   // Error por si no existe ningun acta con el numero dado
   if( error == 0 ){
...
void Universidad::eliminarActa(){
   ...
   cout << "Ingrese el codigo del acta: \n";
   cin >> codigoSelect;
   // Un ciclo para recorrer las actas y verificar si existe el acta
   for( i = 0; i < vectorActasTrabajos.size(); i++){
      if ( vectorActasTrabajos[i].getNumeroActa() == codigoSelect ){
         // Verifica si el estado del acta esta abierto, para así poder eliminarlo
   ...
   // Error por si no se encuentra acta con el numero dado
   if( error == 0 ){
      cout << endl << "No se encontro ningun acta." << endl;
   }
   system("PAUSE()");
...
void Universidad::crearTxt(){
   ...
   cout << "Ingrese el codigo del acta: \n";
   cin >> codigoSelect;
   //verifica el codigo seleccionado para empezar enviar los datos a el .txt
   for( i = 0; i < vectorActasTrabajos.size(); i++){ 
      if ( vectorActasTrabajos[i].getNumeroActa() == codigoSelect ){
         ...
         // Un ciclo que recorre todos los criterios enviandolos al archivo txt
         for( j = 0; j < 8; j++){
   ...
   // Error por si no se encuentra ningun acta
   if( error == 0 ){
      cout << endl << "No se encontro ningun acta." << endl;
   }
   system("PAUSE()");
